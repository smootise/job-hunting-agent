...
- **Phase 0–2 done, most of Phase 3 done, and the webapp (v1 + v2) shipped.** Phase 2 = hard filters + LinkedIn description enrichment + address/commute enrichment + LLM scoring. Phase 3 = the shared agent tool loop + the two **research agents** (address + company) shipped; **the cover-letter agent is the one remaining Phase 3 piece and the natural next task** (it was paused to build the webapp). The webapp (`jobscout serve`) shipped in two layers on top of the pipeline: v1 read views (dashboard / list / detail) and v2 writes (application tracking + run-from-UI buttons) — see the Webapp bullet below and `docs/webapp.md`. LLM scoring shipped per two honored decisions (`docs/architecture.md`, `docs/scoring.md`): the scorer owns remote-policy inference for the silent offers, and `weekly_commute_fit` is computed in **Python** from `commute_minutes` via an **owner-calibrated curve** (12 ranked scenarios), not by the LLM. Address/commute enrichment shipped via **Google Routes for all modes** (transit + traffic-aware driving + bike), a deliberate deviation from the brief's PRIM+OSRM plan — full rationale + security invariants in `docs/enrichment.md`. See the brief for the phase plan.
...
- **Webapp:** `jobscout serve` (default `http://127.0.0.1:8020`; `--host`/`--port`/`--reload`) runs a **FastAPI + HTMX** app over `data/jobs.db`. **v1 (read-only):** dashboard / ranked offer list / offer-detail, mounted on `storage/queries.py` (`get_job`, `list_jobs` with **allowlist-guarded** sorting incl. per-criterion via `json_extract` — never interpolate a raw sort string; `weekly_commute_fit` lives at `$.weekly_commute_fit`, not in `$.criteria_scores`; the list view also filters by a **`posted_after` date** — default last-30-days, `posted_at >= ? OR posted_at IS NULL` so the ~1/3 of undated LinkedIn offers are never silently dropped, with a `hide_undated` opt-in — and defaults to **hiding rejected** via a multi-value `statuses=("passed","needs_review")`; the route validates the date shape → 400, echoes controls into `active`, and offers an explicit "All (incl. rejected)" status) and `storage/stats.py` (dashboard counts that **mirror the `select_jobs_to_*` worklist predicates**, guarded by a parity test). htmx is **vendored locally** (`web/static/`, no CDN). **v2 (writes):** *application tracking* — a **separate** `offer_review(job_id, disposition, applied_at, notes, updated_at)` table (human-owned, kept out of the pipeline-owned `jobs`; `db.upsert_review` preserves the original `applied_at` across edits; `list_jobs`/`get_job` LEFT JOIN it, `stats.review_stats` counts it), and *run-CLI buttons* — a single-worker in-process `web/runner.py` (`JobRunner`, started via a `lifespan`) that runs the **same `pipeline.run_*` code** the CLI does in a background thread, serialized (one SQLite writer), with a per-offer progress bar (an additive `on_progress(done,total)` param — `pipeline.ProgressFn` — on every `run_*`, default `None` so the CLI is unaffected). "Run whole pipeline" halts on the first stage that *raises* and **resumes for free** on re-press (idempotency); per-row fail-soft never halts. A **Stop** button (`POST /runs/cancel`) requests a **cooperative cancel**: a `threading.Event` checked inside the reused `on_progress` closure — once per offer, *after* it commits — raises `RunCancelled`, aborting the chain with status `cancelled`; the in-flight offer isn't wasted and a re-run resumes for free (so "Stop + re-run" *is* pause). Granularity is one offer; a hung LLM call still needs `Ctrl+C`. `/runs/cancel` is registered **before** the `/runs/{stage}` catch-all so "cancel" isn't parsed as a stage. The offer-detail page also has **per-offer** buttons (`POST /offers/{id}/runs/{stage}`) that re-run one stage on one offer: every worklist selector (`select_unfiltered_jobs`, `select_jobs_to_enrich`, `select_jobs_to_research_*`, `select_jobs_missing_description`, `select_jobs_to_score`) takes an `ids` param that drops the "already done" gate (so a deliberate re-run works) but **keeps each stage's eligibility gate** (a rejected offer no-ops for the pipeline stages — a safety rail, not idempotency); the runner thunks pass `ids=[job_id]`. Button visibility: enrich-linkedin only on `linkedin_email` offers; pipeline stages hidden on rejected offers; re-filter always shown. All V2 write routes are `POST` behind a **same-origin guard** (`web/security.py`); the runner passes explicit `db_path`/`env_path` from Settings (never the CWD default). **Gotcha:** the same-origin guard checks `settings.port`, and `serve` runs the module-level `app` — so `--host`/`--port` are exported as `JOBSCOUT_HOST`/`JOBSCOUT_PORT` before uvicorn imports the app, else writes 403 on a non-default port. **No email anywhere; the runner adds no new external capability; office lat/lon may be shown but home coords are never in `jobs`.** Still deferred: company-location map, multi-stage ATS, live logs. Full detail in `docs/webapp.md`; dev workflow (incl. the `serve --stop` gotcha) in `docs/dev.md`. **Windows gotcha:** a running `jobscout serve` holds a lock on `.venv/Scripts/jobscout.exe`, so `uv sync`/`uv run` fail with "file in use" when the package needs rebuilding — run **`uv run jobscout serve --stop`** first (it writes a PID file to `data/`). Verify webapp changes fast with **`uv run python scripts/webapp_smoke.py`** (drives the routes in-process via TestClient — no server to launch or leak).
