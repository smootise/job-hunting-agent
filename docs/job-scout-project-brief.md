...
4. **Enrich: address & commute** — for survivors only (it costs API calls, so it runs after the cheap filters). Deterministic code except step 4 of the chain. Skip entirely for fully-remote offers. *(As-built note: this shipped with two deliberate deviations from the plan below — routing consolidated onto **Google Routes for all modes** (dropping PRIM + OSRM), and a three-strategy commute model with a rail+bike hybrid. The registry lookup (step 3) and the research agent (step 4) remain Phase 3. See `docs/enrichment.md` for the actual implementation; the text below is the original design intent.)*
